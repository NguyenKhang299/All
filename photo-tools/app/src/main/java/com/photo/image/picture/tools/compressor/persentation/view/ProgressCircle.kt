...
class ProgressCircle : View {
    ...
    private var sweepAngle: Float
        get() = this.currentProgress / (totalProgress / 360)
        set(value) {
            this.currentProgress = value * (totalProgress / 360)
        }
    ...
    private fun drawTextProgress(canvas: Canvas) {
        val text = if (format == null) this.currentProgress.toString() else String.format(
            format!!,
            this.currentProgress
        )
        onProgressChange?.onProgress(this.currentProgress)
        val textBounds = Rect()
    ...
    private fun initial(attrs: AttributeSet?) {
        ...
        this.currentProgress = attrs.getFloat(R.styleable.ProgressCircle_toProgress, 0f)
